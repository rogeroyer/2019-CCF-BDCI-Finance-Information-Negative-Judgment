...
bert_path ='../../PreTrainModel/roeberta_zh_L-24_H-1024_A-16/'    #模型地址
config_path = bert_path + 'bert_config_large.json'
checkpoint_path = bert_path + 'roberta_zh_large_model.ckpt'
dict_path = bert_path + 'vocab.txt'


save_model_path='./model/zy'
save_mdoel_name_pre='robertalarge'
#################################################load data#########################################
...
best_emotions=pd.read_csv(best_emotions_path)   #

print(test.shape)
print(sub.shape)
print(best_emotions.shape)

sub['n_pred']=sub['preb1'].apply(lambda x: int(x>0.5))
sub['l_pred']=sub['preb2'].apply(lambda x: int(x>0.5))


#entity_label list
temp=sub[['id','entity_label']].groupby('id')['entity_label'].agg(lambda x:list(x))
test['entity_label_list']=test['id'].map(temp)


#实体的概率
temp=sub[['id','preb2']].groupby('id')['preb2'].agg(lambda x:list(x))
test['l_pred_list']=test['id'].map(temp)

temp=sub[['id','l_pred']].groupby('id')['l_pred'].agg(lambda x:list(x))
test['l_label_list']=test['id'].map(temp)

#情感的值
temp=sub[['id','n_pred']].groupby('id')['n_pred'].agg(lambda x:list(x))
test['n_label_list']=test['id'].map(temp)
...
test['preb_entity']=list(map(lambda x,y,z:get_negative_and_entity(x,y,z),
                             test['n_label_list'],test['l_label_list'],test['entity_label_list']))

test['pred_negative']=test['preb_entity'].apply(lambda x: 1 if str(x)!='nan' else 0)

test['negative']=best_emotions['negative'].values


print(test.columns)

def right_emotion(best_negative,pred_negative,preb_entity,entity_label_list,l_pred_list):
    #标签一致
    if best_negative==pred_negative:
        return preb_entity
    #最好情感为0
    if best_negative==0:
        return np.nan
    #最好情感为1
        ...
        print(entity_label_list[np.argmax(np.array(l_pred_list))])
        return entity_label_list[np.argmax(np.array(l_pred_list))]



test['key_entity']=list(map(lambda best_negative,pred_negative,preb_entity,entity_label_list,l_pred_list:
                        right_emotion(best_negative, pred_negative, preb_entity, entity_label_list, l_pred_list),
                              test['negative'],test['pred_negative'],test['preb_entity'],test['entity_label_list'],test['l_pred_list']
                              ))


sub=test[['id','negative','key_entity']]

sub.to_csv('result/robertalarge_result_mean.csv',index=False)

print(sub['negative'].value_counts())
print(best_emotions['negative'].value_counts())
