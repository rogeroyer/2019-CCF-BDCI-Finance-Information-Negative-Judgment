...
bert_path = '../../PreTrainModel/roeberta_zh_L-24_H-1024_A-16/' 
...
def text_truncate(row):
    ...
    if title_first_index==-1 and text_first_index>480: # 若实体没在title中出现，且在text中的480字之后出现，则进行截取
        return row['text'][text_first_index-200:text_first_index+300] # 实体前截取200个字，实体后截取300个字
    else:
        return row['text']
...
def run_cv(nfolds, data, data_label, data_test, epochs=10, date_str='1107'):
    ...
    for i, (train_fold, test_fold) in enumerate(skf):
        ...
        model.fit_generator(
            ...
            callbacks=[early_stopping, plateau, checkpoint], # evaluator, 
            verbose=2
        )
...
np.save('weights/keras_weight/fusai/train' + model_name + strategy + folds_num + date_str + '.npy', train_model_pred)
np.save('weights/keras_weight/fusai/test' + model_name + strategy + folds_num + date_str + '.npy', test_model_pred)
...
test_2 = pd.read_csv('../../dataSet/round2_test.csv', encoding='utf-8') # 导入测试集
submit = test_2[['id']]
...
# post-process：情感修正
# before = pd.read_csv('submission/emotion_voting_three_models_39215985.csv', encoding='utf-8')
...
res = test.copy()[['id', 'entity_label']]
res['prob'] = test_model_pred[:, 1]
res = res[res['id'].isin(id_set)]
res = res.sort_values(by=['id', 'prob'], ascending=False).drop_duplicates('id', keep='first')
...
test_3 = pd.read_csv('../../dataSet/round2_test.csv', encoding='utf-8')
su = test_3[['id']]
...
time_now = datetime.datetime.now().strftime('%Y%m%d_%H%M%S')
su.to_csv('submission/RoBERTa-large_ccy.csv', encoding='utf-8', index=None)